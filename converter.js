  ...
  for (let i = 0; i < consonants.length; i++) {
    ...
    for (let j = vowels.length - 1; j >= 0; j--) {
      ...
      string = string.replaceAll(
        consonant + vowels[j],
        String.fromCharCode(0xe111 + 0x11 * i + j),
      );
    }
  }
...
const erangLatin =
  "1234567890-qwertyuiopasdfghjklzxcvbnm,.!@#$%^&*()_QWERTYUIOPASDFGHJKLZXCVBNM<>";
const erangErang =
  "";
...
let zasospiquaMap = {};
fetch("./zasospiqua_map.json")
  .then((data) => data.json())
  .then((json) => {
    zasospiquaMap = json;
  });

const zasospiquaReplaces = {
  ".": "。",
  ",": "、",
  "!": "！",
  "?": "？",
...
let i = 0;
function hanaravitezasospiqua(string) {
  i = 0;
  extra.value = "";

  Object.keys(zasospiquaReplaces).forEach((key) => {
    let value = zasospiquaReplaces[key];
  ...
  Object.keys(zasospiquaMap)
    .sort((a, b) => b.length - a.length)
    .forEach((key) => {
      let value = zasospiquaMap[key];
      if (string.toLowerCase().endsWith(key.toLowerCase())) {
        extra.value += `${++i}. ${key}: ${value}\n`;
